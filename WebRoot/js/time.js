...
function SelectDate(obj,strFormat)
{
    ...
    cal = (cal==null) ? new Calendar(by, ey, 0) : cal;    //不用每次都初始化 2006-12-03 修正   //默认为中文 modified by qingfeng_li
    ...
    if (dateArray[i] != "&nbsp;"){
      tds[i].onclick = function () {
        if (calendar.dateControl != null){
         ...
         _date_callback(calendar.dateControl,calendar);//add by qingfeng_li 选择日期后添加回调方法 2013-5-28
        }
        calendar.hide();
      }
...
Calendar.prototype.getMonthViewArray = function (y, m) {
  var mvArray = [];
  var dayOfFirstDay = new Date(y, m, 3).getDay(); //modified by qingfeng_li 日历控件的第一天,为1990-1-1 星期三（所以这里为3）
...
//add by qingfeng_li 选择日期后添加回调方法 2013-5-28 
//control为当前控件，calendar为所选日历
function _date_callback(control,calendar){
//calendar.date.getFullYear() 年
//calendar.date.getMonth() 月
//calendar.dateFormatStyle 日期格式
}